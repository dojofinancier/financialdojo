...
import {
  createCohortFAQAction,
  updateCohortFAQAction,
  deleteCohortFAQAction,
  getCohortFAQsAction,
  reorderCohortFAQsAction,
} from "@/app/actions/cohort-faqs";
...
interface CohortFAQManagementProps {
  cohortId: string;
}

export function CohortFAQManagement({ cohortId }: CohortFAQManagementProps) {
  ...
  const loadFAQs = useCallback(async () => {
    try {
      setLoading(true);
      const result = await getCohortFAQsAction(cohortId);
  ...
  const handleCreate = async () => {
    ...
    try {
      const result = await createCohortFAQAction(cohortId, {
  ...
  const handleUpdate = async (id: string) => {
    ...
    try {
      const result = await updateCohortFAQAction(id, {
        question: formData.question,
        answer: formData.answer,
      });
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Add and manage frequently asked questions for this cohort
          </CardDescription>
        </CardHeader>
          ...
          <div className="border rounded-lg p-4 space-y-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="e.g., What's the maximum number of participants?"
              />
            </div>
            <div className="space-y-2">
