...
const BATCH_SIZE = 50; // Reverted to proven working batch size

// Initialize PG Client for SOURCE database
const sourceClient = new Client({
    connectionString: process.env.DIRECT_URL, // Use DIRECT_URL for source
});
...
async function migrateChunks() {
    try {
        ...
        while (processedChunks < totalChunks) {
            ...
            `;
                ...
                for (const row of batch) {
                    ...
                    values.push(row.chunk_id);
                    rowParams.push(`$${paramIndex++}`);

                    values.push(row.chunk_text);
                    rowParams.push(`$${paramIndex++}`);
                    ...
                    if (row.embedding) {
                        // Start of vector '[', end ']'
                        rowParams.push(`'${row.embedding}'::vector`);
                    } else {
                        rowParams.push(`NULL`);
                    }

                    values.push(row.token_count);
                    rowParams.push(`$${paramIndex++}`);

                    values.push(row.section_path);
                    rowParams.push(`$${paramIndex++}`);

                    values.push(row.document_id);
                    rowParams.push(`$${paramIndex++}`);

                    values.push(row.content_hash);
                    rowParams.push(`$${paramIndex++}`);

                    values.push(row.source_type);
                    rowParams.push(`$${paramIndex++}`);

                    values.push(row.http_status);
                    rowParams.push(`$${paramIndex++}`);

                    values.push(row.retrieved_at);
                    rowParams.push(`$${paramIndex++}`);

                    values.push(row.version);
                    rowParams.push(`$${paramIndex++}`);

                    values.push(row.source_id);
                    rowParams.push(`$${paramIndex++}`);

                    values.push(row.canonical_url);
                    rowParams.push(`$${paramIndex++}`);

                    values.push(row.final_url);
                    rowParams.push(`$${paramIndex++}`);

                    values.push(row.title);
                    rowParams.push(`$${paramIndex++}`);

                    values.push(row.tier);
                    rowParams.push(`$${paramIndex++}`);

                    values.push(row.reliability_score);
                    rowParams.push(`$${paramIndex++}`);
                    ...
                    // Add unique placeholder for this row
                    valueStrings.push(`(${rowParams.join(',')})`);
                }

                const insertQuery = `
                INSERT INTO primary_store_chunks (
                    chunk_id, chunk_text, chunk_index, embedding, 
                    token_count, section_path, document_id, content_hash, source_type, 
                    http_status, retrieved_at, version, source_id, canonical_url, 
                    final_url, title, tier, reliability_score, language, status, 
                    course, metadata, created_at
            ...
            `;
                ...
                // Log every ~5% or at least every 1000
                if (processedChunks % 1000 < BATCH_SIZE || processedChunks >= totalChunks) {
        ...
        // --- 2. Recreate Index ---
        console.log('Recreating index idx_primary_store_chunks_embedding (this might take a while)...');
    ...
    `);
        ...
        while (processedElements < totalElements) {
            ...
            `, values);
                ...
                if (processedElements % 1000 < BATCH_SIZE || processedElements >= totalElements) {
