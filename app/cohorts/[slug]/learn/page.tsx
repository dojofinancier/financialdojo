...
async function CohortLearningContent({
  ...
  if (!("data" in result) || !result.success || !result.data) {
    ...
    console.error(
      `[CohortLearningPage] Failed to load cohort content for slug ${slug}. Error: ${errorMessage}`,
    );
    if (errorMessage?.includes("access") || errorMessage?.includes("inscrit")) {
      console.log(
        "[CohortLearningPage] Access denied or not enrolled, redirecting to /dashboard/student",
      );
