...
import fs from "fs";
import { getReceiptDataForAdminAction } from "@/app/actions/payments";
...
export async function GET(
  ...
  try {
    ...
    const result = await getReceiptDataForAdminAction(paymentIntentId);
    ...
    await logServerError({
      errorMessage: `Admin receipt PDF generation failed: ${error instanceof Error ? error.message : "Unknown error"}`,
